...
int M, N, P, K, t_size=2, i, dom_debug=1;
  ...
  ofstream fout("cpu_timing_serial_chrono_float.txt",ios::out|ios::app);
  ...
  #endif
  {
    ...
    if(dom_debug) printf("case 1.1 batch in p\n");
     ...
     for(i=0;i<10;i++) {
    cblas_batch_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
		      M, N, K, alpha,
		      h_A, M, 0,
		      h_B, K, K*N, beta,
		      h_C, M, M*N,
  ...
  time_span = duration_cast<duration<double>>(t2-t1);         
     time_11_b_p = (float)time_span.count()/10*1000;
    ...
    cblas_batch_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
     ...
     t2 =  high_resolution_clock::now(); 
     time_span = duration_cast<duration<double>>(t2-t1);         
    time_11_b_n = (float)time_span.count()/10*1000;
    if(dom_debug) {printf("C:\n"); print_ten(M,N,P,h_C);}
    free(h_A); free(h_B); free(h_C);
    ...
    cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
     ...
     t2 =  high_resolution_clock::now(); 
  time_span = duration_cast<duration<double>>(t2-t1);         
    time_11_f = (float)time_span.count()/10*1000;
    if(dom_debug) {printf("C:\n"); print_ten(M,N,P,h_C);}
    free(h_A); free(h_B); free(h_C);
    ...
    cblas_batch_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
     ...
     t2 =  high_resolution_clock::now(); 
 time_span = duration_cast<duration<double>>(t2-t1);         
    time_12_b_p = (float)time_span.count()/10*1000;
    if(dom_debug) {printf("C:\n"); print_ten(M,N,P,h_C);}
    free(h_A); free(h_B); free(h_C);
    ...
    cblas_batch_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
     ...
     t2 =  high_resolution_clock::now(); 
    time_span = duration_cast<duration<double>>(t2-t1);         
    time_12_b_n = (float)time_span.count()/10*1000;
    if(dom_debug) {printf("C:\n"); print_ten(M,N,P,h_C);}
    free(h_A); free(h_B); free(h_C);
    ...
    t1 =  high_resolution_clock::now();    
    for(i=0;i<10;i++) {
    cblas_batch_sgemm(CblasColMajor, CblasTrans, CblasNoTrans,
		      M, N, K, alpha,
		      h_A, K, 0,
		      h_B, K, K*N, beta,
		      h_C, M, M*N,
     ...
     t2 =  high_resolution_clock::now(); 
    time_span = duration_cast<duration<double>>(t2-t1);         
    time_21_b_p = (float)time_span.count()/10*1000;
    if(dom_debug) {printf("C:\n"); print_ten(M,N,P,h_C);}
    free(h_A); free(h_B); free(h_C);
    ...
    time_21_b_n = (float)time_span.count()/10*1000;
    if(dom_debug) {printf("C:\n"); print_ten(M,N,P,h_C);}
    free(h_A); free(h_B); free(h_C);
    ...
    cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans,
     ...
     t2 =  high_resolution_clock::now(); 
     time_span = duration_cast<duration<float>>(t2-t1);         
    time_21_f = (float)time_span.count()/10*1000;
    if(dom_debug) {printf("C:\n"); print_ten(M,N,P,h_C);}
    free(h_A); free(h_B); free(h_C);
    ...
    cblas_batch_sgemm(CblasColMajor, CblasTrans, CblasNoTrans,
     ...
     t2 =  high_resolution_clock::now(); 
     time_span = duration_cast<duration<double>>(t2-t1);         
    time_22_b_p = (float)time_span.count()/10*1000;
    if(dom_debug) {printf("C:\n"); print_ten(M,N,P,h_C);}
    free(h_A); free(h_B); free(h_C);
    ...
    cblas_batch_sgemm(CblasColMajor, CblasTrans, CblasNoTrans,
     ...
     t2 =  high_resolution_clock::now(); 
 time_span = duration_cast<duration<double>>(t2-t1);         
    time_22_b_n = (float)time_span.count()/10*1000;
    if(dom_debug) {printf("C:\n"); print_ten(M,N,P,h_C);}
    free(h_A); free(h_B); free(h_C);

    h_A = (float*)malloc(M*K*sd); h_B = (float*)malloc(N*K*P*sd); h_C = (float*)malloc(M*N*P*sd); // host variables
    h_tmp_B = (float*)malloc(K*N*P*sd); // for btas transposes
    for(i=0; i<M*K; ++i) if(dom_debug) h_A[i] = i+1; else h_A[i] = rand()/(float)RAND_MAX;
    ...
    if(dom_debug) printf("case 1.3 batch in p\n");
     t1 =  high_resolution_clock::now();    
     for(i=0;i<10;i++) {
    cblas_batch_sgemm(CblasColMajor, CblasNoTrans, CblasTrans,
		      M, N, K, alpha,
		      h_A, M, 0,
		      h_B, N, N*K, beta,
		      h_C, M, M*N,
     ...
     t2 =  high_resolution_clock::now(); 
     time_span = duration_cast<duration<double>>(t2-t1);         
    time_13_b_p = (float)time_span.count()/10*1000;
    if(dom_debug) {printf("C:\n"); print_ten(M,N,P,h_C);}
    t1 =  high_resolution_clock::now();    
    for(i=0;i<10;i++) {
    for(int p=0; p<P; p++) // timing explicit transpose: btas-like
      mkl_somatcopy('C', 'T', N, K, alpha, h_B+(N*K*p), N, h_tmp_B+(K*N*p), K);
    }
    t2 =  high_resolution_clock::now(); 
    time_span = duration_cast<duration<double>>(t2-t1);         
    time_13_geam = (float)time_span.count()/10*1000;
    free(h_tmp_B);
    free(h_A); free(h_B); free(h_C);
    ...
    t2 =  high_resolution_clock::now(); 
     time_span = duration_cast<duration<double>>(t2-t1);         
    time_15_b_p = (float)time_span.count()/10*1000;
    if(dom_debug) {printf("C:\n"); print_ten(M,N,P,h_C);}
    free(h_A); free(h_B); free(h_C);
   ...
   t2 =  high_resolution_clock::now(); 
  time_span = duration_cast<duration<double>>(t2-t1);         
    time_15_f = (float)time_span.count()/10*1000;
    if(dom_debug) {printf("C:\n"); print_ten(M,N,P,h_C);}
    free(h_A); free(h_B); free(h_C);
    ...
    t1 =  high_resolution_clock::now();    
    for(i=0;i<10;i++) {
    cblas_batch_sgemm(CblasColMajor, CblasTrans, CblasTrans,
		      M, P, K, alpha,
		      h_B, K, K*M,
		      h_A, P, 0, beta,
		      h_C, M*N, M,
     ...
     t2 =  high_resolution_clock::now(); 
     time_span = duration_cast<duration<double>>(t2-t1);         
    time_51_b_n = (float)time_span.count()/10*1000;
    if(dom_debug) {printf("C:\n"); print_ten(M,N,P,h_C);}
    free(h_A); free(h_B); free(h_C);
    ...
    t2 =  high_resolution_clock::now(); 
    time_span = duration_cast<duration<double>>(t2-t1);         
    time_51_f = (float)time_span.count()/10*1000;
    if(dom_debug) {printf("C:\n"); print_ten(M,N,P,h_C);}
    free(h_A); free(h_B); free(h_C);
    ...
    cblas_batch_sgemm(CblasColMajor, CblasTrans, CblasNoTrans,
     ...
     t2 =  high_resolution_clock::now(); 
     time_span = duration_cast<duration<double>>(t2-t1);         
    time_61_b_n = (float)time_span.count()/10*1000;
    if(dom_debug) {printf("C:\n"); print_ten(M,N,P,h_C);}
    free(h_A); free(h_B); free(h_C);
    ...
    t2 =  high_resolution_clock::now(); 
     time_span = duration_cast<duration<double>>(t2-t1);         
    time_61_f = (float)time_span.count()/10*1000;
    if(dom_debug) {printf("C:\n"); print_ten(M,N,P,h_C);}
    free(h_A); free(h_B); free(h_C);
